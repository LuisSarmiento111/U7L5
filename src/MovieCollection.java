...
public class MovieCollection
{
    ...
    private void searchCast()
    {
        ...
        for (int i = 0; i < movies.size(); i++) {
            ...
            for (int x = 0; x < cast.length; x++) {
                String actor = cast[x];
                if (!actors.contains(actor) && actor.toLowerCase().contains(searchTerm)) {
                    actors.add(actor);
                }
            }
        }
        ...
        int choice = scanner.nextInt();
        scanner.nextLine();
        String selectedActor = actors.get(choice - 1).toLowerCase();

        for (int i = 0; i < movies.size(); i++) {
            String[] keywords = movies.get(i).getCast().split("\\|");
            for (int x = 0; x < keywords.length; x++) {
                String actor = keywords[x].toLowerCase();
        ...
        Movie movie = results.get(choice - 1);
    ...
    private void searchKeywords()
    {
        ...
        for (int i = 0; i < movies.size(); i++) {
            ...
            for (int x = 0; x < keywords.length; x++) {
                String keyword = keywords[x].toLowerCase();
                if (keyword.equals(searchTerm)) {
                    results.add(movies.get(i));
                }
            }
        }
    ...
    private void listGenres()
    {
        ...
        for (int i = 0; i < movies.size(); i++) {
            ...
            for (int x = 0; x < genres.length; x++) {
                String genre = genres[x].toLowerCase();
        ...
        System.out.println("Which genre you would like to find movies on");
        System.out.print("Enter number: ");

        int choice = scanner.nextInt();
        scanner.nextLine();

        String selectedGenre = allGenres.get(choice - 1).toLowerCase();
        ArrayList<Movie> results = new ArrayList<Movie>();
    ...
    private void listHighestRated()
    {
        ...
        for (int i = 0; i < topRated.size(); i++) {
            int maxIndex = i;
            double max = topRated.get(i).getUserRating();
            for (int j = i; j < topRated.size(); j++) {
                if (topRated.get(j).getUserRating() >= max) {
                    maxIndex = j;
                    max = topRated.get(j).getUserRating();
                }
            }
            topRated.add(i, topRated.remove(maxIndex));
        }
        for (int i = 50; i < topRated.size(); i++) {
            topRated.remove(i);
            i--;
        }
        for (int i = 0; i < topRated.size(); i++) {
    ...
    private void listHighestRevenue()
    {
        ...
        for (int i = 0; i < topRevenue.size(); i++) {
            int maxIndex = i;
            double max = topRevenue.get(i).getRevenue();
            for (int j = i; j < topRevenue.size(); j++) {
                if (topRevenue.get(j).getRevenue() >= max) {
                    maxIndex = j;
                    max = topRevenue.get(j).getRevenue();
                }
            }
            topRevenue.add(i, topRevenue.remove(maxIndex));
        }
        for (int i = 50; i < topRevenue.size(); i++) {
            topRevenue.remove(i);
            i--;
        }
        for (int i = 0; i < topRevenue.size(); i++) {
